// File-based store (in-memory Maps + JSON persistence).
// No native modules — safe on Vercel serverless. Login is guaranteed via the
// env-seeded account (SEED_EMAIL / SEED_PASSWORD_HASH) even on cold starts.
...
// Prefer ./storage; fall back to the OS temp dir when the bundle is read-only (Vercel).
function resolveDataFile() {
  const preferredDir = path.join(__dirname, 'storage');
  try {
    if (!fs.existsSync(preferredDir)) fs.mkdirSync(preferredDir, { recursive: true });
    fs.accessSync(preferredDir, fs.constants.W_OK);
    return path.join(preferredDir, 'users.json');
  } catch {
    return path.join(os.tmpdir(), 'elite-users.json');
  }
}

const DATA_FILE = process.env.DATA_FILE || resolveDataFile();
  ...
  try {
    const data = {
      users: Array.from(users.values()).map(u => ({
        ...
        createdAt: u.createdAt ? (u.createdAt.toISOString ? u.createdAt.toISOString() : u.createdAt) : null,
        chats: u.chats || [],
    ...
    // Read-only filesystem (e.g. Vercel) — keep running with in-memory data.
    console.warn('[store] saveToDisk skipped:', err.message);
  }
}
...
function toSafeUser(user) {
  if (!user) return null;
  const { passwordHash, chats: _c, documents: _d, favorites: _f, ...safe } = user;
  return safe;
}
...
exports.findUserByEmail = (email) => {
  for (const u of users.values()) {
    if (u.email === String(email || '').toLowerCase().trim()) return u;
  }
  return null;
};

exports.findUserById = (id) => users.get(String(id)) || null;
...
exports.getChat = (chatId) => chats.get(String(chatId)) || null;
...
exports.deleteChat = (chatId) => {
  ...
  if (user) user.chats = user.chats.filter(id => id !== String(chatId));
...
exports.addChatMessage = (chatId, role, content) => {
  const chat = chats.get(String(chatId));
  if (!chat) return null;
  const message = { id: String(Date.now()) + Math.random().toString(36).slice(2, 6), role, content, createdAt: new Date() };
...
;(function seedFromEnv() {
  ...
  const id = 'seed-1';
  users.set(id, {
    id, email, passwordHash: hash,
    firstName: 'حساب', lastName: 'تجريبي', phone: '', accountType: 'individual', governorate: '',
    createdAt: new Date(), chats: [], documents: [], favorites: [],
  });
  saveToDisk();
})();
